...
public class CommentsService {
  ...
  public CommentResponseDto createComment(CommentRequestDto request) {
    ...
    if (user.getRole().name().equals("ROLE_USER")) {
      ...
      pointService.addPoints(user.getUserId(), Point.WRITE_COMMENT_POINT.getAmount(),
          "댓글 작성 포인트 지급");
    } else {
      ...
      pointService.addPoints(user.getUserId(), Point.WRITE_COMMENT_POINT.getAmount() * 2,
          "댓글 작성 포인트 지급");
    }
  ...
  public void deleteComment(Long commentId, Long userId) {
    ...
    Users user = userRepository.findById(userId)
        .orElseThrow(() -> new IllegalArgumentException("유저 ID를 찾을 수 없습니다."));

    if (!comment.getUser().getUserId().equals(user.getUserId()) && !user.getRole().name()
        .equals("ROLE_ADMIN")) {
