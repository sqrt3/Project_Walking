...
function updateBoardContent(boardId) {
  if (Number(boardId) === 3 && userNickname == null) {
    ...
    const nicknameOption = document.querySelector(
        '#searchCategory option[value="nickname"]');
    if (Number(boardId) === 3) {
      if (nicknameOption) {
        nicknameOption.remove();
      }
    } else {
      if (!nicknameOption) {
  ...
  .then(data => {
    if (data) {
      ...
      const imageUrl = data.imageUrl[0]
          || "https://walkingproject1.s3.ap-northeast-2.amazonaws.com/c07aa2d0-formi.jpg";
...
function displayPagination(searchMode) {
  ...
  if (pageCount > pagesToShow && currentPage > 10) {
    ...
    prevButton.addEventListener('click', function (event) {
      ...
      searchMode ? performSearch(document.getElementById('boardId').value,
          currentPage) : fetchRecentPosts(
          document.getElementById('boardId').value, currentPage);
      displayPagination(searchMode);
    });
    pagination.appendChild(prevButton);
  }

  const startPage = Math.floor((currentPage - 1) / pagesToShow) * pagesToShow
      + 1;
